...
class CUserKnightsRankSet : public CMyRecordSet<T>
{
public:
	CUserKnightsRankSet(MapType *stlMap, CDatabase* pDatabase = NULL)
		: CMyRecordSet<T>(pDatabase), m_stlMap(stlMap)
	{
		m_nFields = 4;
	}

	DECLARE_DYNAMIC(CUserKnightsRankSet)
	virtual CString GetDefaultSQL() { return _T("[dbo].[USER_KNIGHTS_RANK]"); };

	virtual void DoFieldExchange(CFieldExchange* pFX)
	{
		pFX->SetFieldType(CFieldExchange::outputColumn);

		RFX_Int(pFX, _T("[shIndex]"), m_data.nRank);
		RFX_Long(pFX, _T("[nMoney]"), m_data.nSalary);
...
#undef MapType
#undef T
IMPLEMENT_DYNAMIC(CUserKnightsRankSet, CRecordset)